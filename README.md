...
This POSIX-shell test launches version `4.0.3`, performs only the MCP handshake and `tools/list`,
...
A successful response identifies `lex-mcp` version `4.0.3` and returns Lex's tool list. Review the
...
all `4.0.3`; the Node range is `>=24`.
...
```bash
LEX_MCP_LEX_TARBALL=/absolute/path/to/smartergpt-lex-4.0.3.tgz npm run test:pack
```
